...
COPY services/nuclios-server/requirements.txt /app/product/requirements.txt
...
ADD services/nuclios-server/api /app/product/api
COPY services/nuclios-server/run_product_webserver_worker.sh /app
COPY services/nuclios-server/db_migrate.sh /app/product
COPY services/nuclios-server/hypercorn_config.toml /app/product
COPY services/execution/widget_factory_lite_module/codex_widget_factory_lite /app
COPY services/execution/widget_factory_lite_module/codex_widget_factory_lite /app/product/codex_widget_factory_lite
COPY services/nuclios-server/sshd_config /etc/ssh/
RUN chmod 777 /app/run_product_webserver_worker.sh
RUN service ssh start
EXPOSE 80 2222
CMD ["/bin/bash", "-c",  "/app/run_product_webserver_worker.sh"]
