...
from api.models.base_models import (  # ExecutionEnvironmentAppMapping,
    ...
    ExecutionEnvironmentApprovalStatus,
...
class ExecutionEnvironmentDao(BaseDao):
    ...
    def get_paginated_execution_environments(
        self, request, page, page_size, search, env_type, env_category, project_id, approval_status
    ):
        try:
            query = (
                ...
                .filter(ExecutionEnvironment.deleted_at.is_(None), ExecutionEnvironment.is_active.is_(True))
            )
            if project_id:
                query = query.join(
                    ExecutionEnvironmentProjectMapping, ExecutionEnvironmentProjectMapping.env_id == ExecutionEnvironment.id
                ).filter(
            ...
            if approval_status:
                query = query.join(
                    ExecutionEnvironmentApprovalStatus, ExecutionEnvironmentApprovalStatus.env_id == ExecutionEnvironment.id
                ).filter(ExecutionEnvironmentApprovalStatus.approval_status == approval_status)
            if search:
                # Search Filter
            ...
            raise GeneralException(
                message="Error occurred while fetching paginated Execution Environments.",
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

    def get_execution_environments(self, request, search, env_type, env_category, project_id, approval_status=None):
        try:
            query = (
                self.db_session.query(ExecutionEnvironment)
                .options(
    ...
    def update_approval_status(self, user, id, execution_environment=None, request_data=None):
        try:
            # Inserting Data in the DB
            if not request_data:
                approval_status = ExecutionEnvironmentApprovalStatus(
                    env_id=id,
                )
                if "id" in user:
                    approval_status.created_by = user["id"]
            else:
                approval_status = self.get_approval_status(id)
                if approval_status:
                    approval_status.approval_status = request_data["action_type"]
                    approval_status.approved_by = user["id"] if "id" in user else None
                else:
                    approval_status = ExecutionEnvironmentApprovalStatus(
                        env_id=id,
                        approval_status=request_data["action_type"],
                        approved_by=user["id"] if "id" in user else None,
                    )
                    self.db_session.add(approval_status)
            self.db_session.add(approval_status)
            self.db_session.flush()
            self.db_session.refresh(approval_status)
            if execution_environment:
                self.db_session.refresh(execution_environment)

            # Update status and logs if action_type is "rejected"
            if request_data and request_data["action_type"] == variables.ApprovalStatus.REJECTED.value:
                execution_env = self.db_session.query(ExecutionEnvironment).filter_by(id=id).first()
                if execution_env:
                    execution_env.status = request_data["action_type"]
                    execution_env.logs = f"Not approved or {request_data['action_type']}"
                    self.db_session.add(execution_env)
                    self.db_session.flush()
                    self.db_session.refresh(execution_env)
            ...
            self.perform_rollback()
            raise GeneralException(
                message="Error occured while updating the approval status for the Execution Environment.",
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

    def get_approval_status(self, env_id):
        try:
            return self.db_session.query(ExecutionEnvironmentApprovalStatus).filter_by(env_id=env_id).first()
        except Exception as e:
            logging.exception(e)
            self.perform_rollback()
            raise GeneralException(
                message="Error occured while fetching Approval status.",
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
