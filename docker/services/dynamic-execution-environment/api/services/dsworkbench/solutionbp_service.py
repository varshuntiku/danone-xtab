...
from api.dtos.execution_environment.solutionbp_dto import SolutionbpDetailDTO
...
class SolutionBlueprintService:
                ...
                if action.action == "add":
                    for payload in action.payload:
                        action_add = self.azure_file_share_service.create_sub_directory(
                            ...
                            directory_path=f"{nodes.project_id}",
                    ...
                    for payload in action.payload:
                        action_remove = await self.azure_file_share_service.delete_directory_tree_from_share(
                            share_name=destination_share_name, directory_path=f"{nodes.project_id}/{payload.path}"
                        )
    ...
    async def save_solution_bp(self, user, share_name, nodes: OnSaveRequest):
        try:
            directory_path = self.azure_file_share_service.get_share_directory_path(share_name, nodes.project_id)
