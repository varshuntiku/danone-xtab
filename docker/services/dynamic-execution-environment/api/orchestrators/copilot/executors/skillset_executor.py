...
class SkillsetExecutor(OrchestratorExecutionBase):
    ...
    def update_copilot(self, user, status, endpoint=None, deployment_logs=None):
    ...
    async def monitor_job_status(self, time=None):
        ...
        while status == "running":
            ...
            job_logs = self.job_manager.get_job_logs(
                job_name=self.fetch_job_name(),
                namespace=self.app_settings.DEPLOYMENT_NAMESPACE,
            )

            self.update_copilot(self.user, "Started", self.skillset_model.get("endpoint"), job_logs)
