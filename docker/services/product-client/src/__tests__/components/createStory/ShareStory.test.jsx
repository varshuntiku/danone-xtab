...
import ShareStory from '../../../components/createStory/ShareStory.jsx';
...
describe('Codex Product test', () => {
    ...
    test('Should render layouts ShareStory Component', async () => {
        getShareableUsers.mockImplementation(({ callback }) =>
            callback([
                { first_name: 'Test', last_name: 'user 1', email: 'testuser1@test.com' },
                { first_name: 'Test', last_name: 'user 2', email: 'testuser2@test.com' },
                { email: 'testuser3@test.com' }
            ])
        );
        getSharedList.mockImplementation(({ callback }) => callback([]));
        const { getByText, debug } = render(
            <Provider store={store}>
                <CustomThemeContextProvider>
                    <Router history={history}>
                        <ShareStory story={story} classes={{}} />
                    </Router>
                </CustomThemeContextProvider>
            </Provider>
        );

        expect(screen.getByTitle('share story')).toBeInTheDocument();
        const shareBtn = screen.getByTitle('share story');
        ...
        act(() => {
            ...
            fireEvent.change(textbox, { target: { value: 'testuser3@test.com' } });
            fireEvent.keyDown(textbox, { key: 'Enter' });
        });
    ...
    test('Should render layouts ShareStory Component 1', async () => {
        getShareableUsers.mockImplementation(({ callback }) =>
            callback([
                { first_name: 'Test', last_name: 'user 1', email: 'testuser1@test.com' },
                { first_name: 'Test', last_name: 'user 2', email: 'testuser2@test.com' }
