...
vi.mock('react-plotly.js', () => ({
    __esModule: true,
    default: (props) => <div data-testid="plotly-mock">{props.children}</div>
}));
...
vi.mock('../../../services/reports', () => ({
    ...
    updateStory: ({ callback }) => callback()
}));

const history = createMemoryHistory();
console.log('Hiistory check', history);
debugger;
describe('Codex Product test', () => {
    afterEach(cleanup);

    test('Should render layouts CreateStory Component', async () => {
        // axios.get.mockResolvedValue(Data)
        const { getByText, debug } = render(
            <Provider store={store}>
                <CustomThemeContextProvider>
                    <Router history={history}>
                        <CreateStory match={{ params: { story_id: 1 } }} />
                    </Router>
                </CustomThemeContextProvider>
            </Provider>
        );
        ...
        expect(screen.getAllByLabelText('delete')).not.toBeNull();
    ...
    test('Should render layouts CreateStory Component', async () => {
        ...
        // expect(screen.getByRole('button',{ name: 'Undo your changes'})).toBeInTheDocument()
        // const undoBtn = screen.getByRole('button',{ name: 'Undo your changes'})
        expect(screen.getByTitle('Undo your changes')).toBeInTheDocument();
        const undoBtn = screen.getByTitle('Undo your changes');
        fireEvent.click(undoBtn);

        //expect(screen.getByText('activeSave is true')).toBeInTheDocument()
        // const saveBtn = screen.getByRole('button',{ name: 'Save your changes'})
        expect(screen.getByTitle('Save your changes')).toBeInTheDocument();
        const saveBtn = screen.getByTitle('Save your changes');
