...
import { selectAllUpdateCheckedNodes } from 'components/dsWorkbench/solutionBluePrint/utils';
...
const onLoadCompleted = (state, action, isFromImport) => {
    ...
    let initial_nodes = [];
    let initial_edges = [...state.current_bp_State.react_flow_data.initial_edges] || [];
    const staticPos = {
        x: 100,
        y: 100
    };
                ...
                data: [
                    {
                        ...
                        treeNode:
                            nodeItem.data[0]?.treeNode?.length > 0 ? nodeItem.data[0].treeNode : [],
                        id: nodeItem.id.toString(),
                        handles: [
                            {
                                type: 'target',
...
const startDownloadAsPng = () => {
    ...
    toPng(element, {
        backgroundColor: localStorage.getItem('codx-products-theme') === 'light' ? 'white' : 'black'
    }).then((dataUrl) => {
...
const solutionBluePrintSlice = createSlice({
    ...
    reducers: {
        ...
        browseBlueprints: (state) => {
            ...
            // state.on_loading = true; // for temp
        ...
        onAddNode: (state, action) => {
            ...
            action.payload.last_node_id && (state.last_node_id = action.payload.last_node_id);
            state.new_node_name = '';
            action.payload.initial_edges && (state.new_edges = action.payload.initial_edges);
        },
        ...
        setZipLinkDownloadStatus: (state, action) => {
            ...
            action.payload?.data?.progress && (progressCount = action.payload.data.progress);
            if (action.payload?.data?.message && action.payload?.data?.status !== 'failed') {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getDirectoryTree.fulfilled, (state, action) => {
                const treeViewData = action.payload?.data?.dir_tree || [];
            ...
            .addCase(importBluePrints.fulfilled, (state, action) => {
                ...
                if (
                    ...
                    if (action.payload?.data?.import_action_list?.length > 0) {
                        state.import_action_list.push(...action.payload.data.import_action_list);
                    }
                }
            })
            ...
            .addCase(onLoadBlueprint.pending, (state) => {
                state.on_loading = true;
                state.on_saved = false;
            })
            .addCase(onLoadBlueprint.fulfilled, (state, action) => {
                state.on_loading = false;
                if (action.payload?.data?.status === 'failed') {
                    state.popup_msg = {
                        open: true,
            ...
            .addCase(onLoadBlueprint.rejected, (state) => {
                state.on_loading = false;
                state.popup_msg = {
                    open: true,
                    message: 'Failed to get solution blueprint data...',
                    severity: 'error'
                };
            })
            .addCase(onSaveBlueprints.pending, (state) => {
                state.on_loading = true;
                state.loading_blueprint_msg = 'Saving...';
            })
            .addCase(onSaveBlueprints.fulfilled, (state, action) => {
                state.is_save_btn_clicked = true;
                state.import_action_list = [];
                state.on_loading = false;
                if (action.payload.data.status === 'success') {
                    state.on_saved = true;
                    state.on_loading = true;
                    state.show_folder_details = false;
                    state.loading_blueprint_msg = 'Re-loading...';
                    state.popup_msg = {
                        open: true,
                    ...
                    state.popup_msg = {
                        open: true,
                        message: action.payload?.data?.message || 'Failed to save blueprints...',
                        severity: 'error'
                    };
                }
            })
            .addCase(onSaveBlueprints.rejected, (state) => {
                state.on_loading = false;
                state.on_saved = false;
                state.popup_msg = {
                    open: true,
            ...
            .addCase(onLoadDefaultBlueprint.pending, (state) => {
                state.on_loading = true;
                state.show_default_blueprint = false;
            })
            .addCase(onLoadDefaultBlueprint.fulfilled, (state, action) => {
                onLoadCompleted(state, action);
                state.show_default_blueprint = false;
                state.import_action_list = [];
            })
            .addCase(onLoadDefaultBlueprint.rejected, (state) => {
                state.on_loading = false;
                state.import_action_list = [];
                state.on_saved = false;
                state.show_default_blueprint = false;
                state.popup_msg = {
                    open: true,
                    message: 'Failed to load default blueprints...',
                    severity: 'error'
                };
            })
