...
import { getGoalData, createGoal, updateGoal } from 'services/connectedSystem_v2';

import * as _ from 'underscore';

class GoalsActions extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            details: {
                name: props.goal?.name ? props.goal.name : '',
                is_active: props.goal?.is_active ? props.goal.is_active : false,
                order_by: props.goal?.order_by ? props.goal.order_by : false,
                objectives: null
            },
            goal_id: props.goal?.id ? props.goal.id : false,
            active_step: 0,
            loading: false
        };
    }
    ...
    setOpen = (value) => {
        ...
        if (value && this.state.goal_id) {
            ...
            getGoalData({
                connSystemGoalId: this.state.goal_id,
                callback: this.onResponseGoalData
            });
        }
    };

    onResponseGoalData = (response) => {
        this.setState({
            details: response,
            loading: false
        });
        ...
        try {
            if (this.props.createGoal) {
                createGoal({
                    ...
                    callback: this.onResponseCreateGoal
                });
            } else {
                updateGoal({
                    payload: this.state.details,
    ...
    onResponseCreateGoal = () => {
        const props = this.props;

        this.handleUpdateResponse(
            props.createGoal ? 'Goal added Successfully' : 'Goal updated Successfully'
        );
        this.setOpen(false);
    ...
    render() {
        const { classes, createGoal } = this.props;
        ...
        return [
            ...
            <Dialog
                ...
                aria-labelledby="create-new-goal"
                aria-describedby="goal-content"
            >
                <DialogTitle className={classes.title} disableTypography id="create-new-goal">
                    <Typography variant="h4" className={classes.heading}>
                        {this.state.goal_id ? 'Edit Goal' : 'Create New Goal'}
                    </Typography>
                    <IconButton
                        title="Close"
                        onClick={() => {
