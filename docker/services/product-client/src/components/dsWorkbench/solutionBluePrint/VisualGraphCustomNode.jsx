...
import { updateCheckedNodes } from './utils';
...
function VisualGraphCustomNode({ data, classes }) {
    ...
    const { treeview_checked_nodes, treeviewNodes, all_treeview_nodes } = solutionBluePrintData;
    ...
    const dynamicCheckBoxClick = (node, isChecked) => {
        ...
        if (isChecked && node.child) {
            ...
            tempNodeList.push(node.nodeId);
            checkNode(node.child, tempNodeList);
            nodeList = treeview_checked_nodes.filter((item) => !tempNodeList.includes(item));
        } else {
            if (isChecked) {
                nodeList = nodeList.concat(treeview_checked_nodes, [node.nodeId]);
            } else {
                nodeList = treeview_checked_nodes.filter((item) => item !== node.nodeId);
            }
        }
        ...
        const updatedAllTreeviewNodes = JSON.parse(JSON.stringify([].concat(all_treeview_nodes)));
