...
def test_model_structure_app_column_types(db_inspector):
    ...
    assert isinstance(columns["theme"]["type"], String)
    assert isinstance(columns["contact_email"]["type"], String)
    assert isinstance(columns["modules"]["type"], String)
    assert isinstance(columns["app_creator_id"]["type"], Integer)
    assert isinstance(columns["is_connected_systems_app"]["type"], Boolean)


def test_model_structure_category_nullable_constraints(db_inspector):
    columns = db_inspector.get_columns(TABLE_NAME)

    expected_nullable_columns = {"id": False, "name": True, "contact_email": True, "theme": True, "modules": True}
...
def test_model_structure_category_column_lengths(db_inspector):
    ...
    assert columns["color"]["type"].length == 10
...
def test_model_structure_foreign_key(db_inspector):
    ...
    expected_foreign_key = {
        "constrained_columns": ["theme_id"],
        "referred_table": "app_theme",
        "referred_columns": ["id"],
    }
