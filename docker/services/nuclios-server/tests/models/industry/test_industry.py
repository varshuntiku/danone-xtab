...
def test_model_structure_industry_column_types(db_inspector):
    ...
    assert isinstance(columns["industry_name"]["type"], String)
    assert isinstance(columns["logo_name"]["type"], String)
    assert isinstance(columns["created_at"]["type"], TIMESTAMP)
    assert isinstance(columns["order"]["type"], Integer)


def test_model_structure_industry_nullable_constraints(db_inspector):
    columns = db_inspector.get_columns(TABLE_NAME)

    expected_nullable_columns = {
        "id": False,
        "industry_name": False,
        "parent_industry_id": True,
        "level": True,
        "logo_name": False,
    }
...
def test_model_structure_industry_default_values(db_inspector):
...
def test_model_structure_category_column_lengths(db_inspector):
    ...
    assert columns["industry_name"]["type"].length == 100
    assert columns["logo_name"]["type"].length == 100
