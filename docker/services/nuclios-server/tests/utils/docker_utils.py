...
def create_database_container(env: str = "local"):
    client = docker.from_env()
    CONTAINER_NAME = "test-db"
    scripts_dir_path = os.path.abspath("./scripts")

    is_container_exists, existing_container = check_container_exists(client, CONTAINER_NAME)
    ...
    time.sleep(1 if env == "local" else 5)
