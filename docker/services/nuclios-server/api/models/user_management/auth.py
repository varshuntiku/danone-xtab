...
class NacRoles(BaseModelMixin):
    ...
    role_permissions = relationship("NacRolePermissions", secondary=nac_role_permissions_identifier, lazy="selectin")
...
@mapper_registry.mapped
class UserLoginAuthCode(BaseModelMixin):
    __tablename__ = "user_login_auth_code"

    email = Column(String(1000), nullable=False, unique=True)
    auth_code = Column(LargeBinary)
    expiry = Column(DateTime)

    def __init__(self, email, auth_code, expiry):
        self.email = email
...
@mapper_registry.mapped
class LoginConfig(BaseModelMixin):
    ...
    def __init__(self, config_name, config_data, is_enabled=False):
