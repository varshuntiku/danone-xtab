...
class GenericHelper:
            ...
            if upload_with_content_type:
                ...
                if file_content_type[0] is not None:
                    ...
                    if dynamic_blob_properties:
                        ...
                        return self.get_blob(final_file_path, time_delta=time_delta)
                else:
    ...
    def upload_metadata_blob(
        ...
        """
        try:
            connection_string = settings.DATA_AZURE_CONNECTION_STRING
            container_name = "saml-metadata"
            ...
            blob_service.create_blob_from_bytes(artifacts_container, final_file_path, file)
            return self.get_blob(final_file_path, "saml-metadata", time_delta)
        except Exception as e:
            logging.exception(e)
            raise GeneralException(
                message={"error": GeneralErrors.BLOB_UPLOAD_ERROR.value},
    ...
    def get_blob(
        self,
        blob_name: str,
        container: str = settings.DATA_FOLDER_PATH,
        time_delta: timedelta = timedelta(hours=1),
    ):
        try:
