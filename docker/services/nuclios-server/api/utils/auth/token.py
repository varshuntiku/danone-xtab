...
from api.models.base_models import UserToken
...
def get_password_token(user_id: int, user_email: str) -> str:
    ...
    return encode_payload(payload, algorithm="HS256")


def validate_execution_token(token: str, get_user_token_by_token_and_email: Callable) -> UserToken | None:
    """
    Validates the passed execution token

    Args:
        token: input token
        get_user_token_by_token_and_email: method to get token details

    Returns:
        User token details or None
    """
    try:
        data = decode(
            jwt=token,
        ...
        user_token_details = get_user_token_by_token_and_email(token, data.get("sub"))

        if user_token_details:
            return user_token_details
        else:
            raise Exception(AuthErrors.INVALID_TOKEN_ERROR.value)
    except Exception as e:
        logging.exception(e)
        raise GeneralException(
            status.HTTP_401_UNAUTHORIZED,
            message={"error": AuthErrors.INVALID_TOKEN_ERROR.value},
        )


def genAccessKey() -> str:
    """
