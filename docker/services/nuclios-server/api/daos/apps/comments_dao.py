...
from api.models.base_models import Approval, Comment, Filter, Notifications, Reply, User
...
class CommentsDao(BaseDao):
    ...
    async def add_comment(
        ...
        try:
            ...
            scenario_list = ",".join(request_data.scenario_list) if request_data.scenario_list is not None else None
            ...
            if request_data.mode == "task":
                approval_ids = await self.add_approval(request_data.approvals, new_comment.id)
            updated_link = await self.create_filter_and_generate_link(
                new_comment, request_data.filters, request_data.link
            )
            if request_data.mode == "task":
                approval_list = []
                for approval in request_data.approvals:
                    email = self.users_doa.get_user_by_id(user_id=approval.user_id).email_address
                    approval_list.append(email)
                    if email in ordered_users:
                        ordered_users.remove(email)
                notifications_list = self.create_notification_objects(
                    ordered_users=approval_list,
                    app_id=request_data.app_id,
                    screen_id=request_data.app_screen_id,
                    user_data=user_data,
                    updated_link=updated_link,
                    widget_name=request_data.widget_name
                    if request_data.widget_id is not None
                    else request_data.screen_name,
                    title="Task Notification",
                    message=f"{user_data.full_name} assigned you a task",
                    type="Task",
                ...
                background_tasks.add_task(
                    ...
                    executor,
                    asyncio.run,
                    self.send_approval_notifications(request_data.approvals, new_comment, updated_link),
                )
            notifications_list = self.create_notification_objects(
    ...
    async def add_reply(
        ...
        try:
            ...
            notifications_list = self.create_notification_objects(
                ...
                screen_id=comment.app_screen_id,
                user_data=user_data,
                updated_link=updated_link,
                widget_name=request_data.widget_name if comment.widget_id is not None else request_data.screen_name,
                title="Comment Notification",
                message=f"{user_data.full_name} mentioned you in a Reply",
                type="Mentions",
                comment_id=request_data.comment_id,
            )
            try:
    ...
    async def get_comments(
        self, app_id, app_screen_id, widget_id, include_deleted, bookmarked, resolved, unresolved, user_id
    ) -> CommentsResponseSchema:
        """
        ...
        """
        try:
            query = self.db_session.query(Comment).filter_by(app_id=app_id, app_screen_id=app_screen_id)
            if widget_id is not None:
                query = query.filter_by(widget_id=widget_id)
            else:
                query = query.filter_by(widget_id=None)
            ...
            comments_dto = [
                CommentDTO(
                    comment, user_id, self.notification_subscription_dao, self.get_approvals(comment_id=comment.id)
                ).__dict__
    ...
    async def send_comment_notifications(self, comment: Comment, updated_link: str):
        try:
            unique_user_ids = []
            approvals = self.get_approvals(comment_id=comment.id)
            if comment.tagged_users:
                tagged_user_ids = json.loads(comment.tagged_users)
                unique_user_ids = list(set(tagged_user_ids))
            for approval in approvals:
                if approval.user_id in unique_user_ids:
                    unique_user_ids.remove(approval.user_id)
            subscripitons = self.notification_subscription_dao.get_subscriptions_based_on_app_id(
                app_id=comment.app_id, screen_id=comment.app_screen_id
            )
            for subscription in subscripitons:
                if subscription.subscription_setting == "@mentions":
                    user = self.db_session.query(User).filter_by(id=subscription.created_by).first()
                    if user.id in unique_user_ids:
                        ...
                        email_data = {
                            ...
                            "link": updated_link,
                            "type": "comment",
                            "message": f"{comment.user_name} has mentioned you in a comment",
                        ...
                        try:
                            ...
                            logging.exception(f"Failed to send email to {user.email_address}: {e}")
                        unique_user_ids.remove(user.id)
                elif subscription.subscription_setting == "all comments":
                    user = self.db_session.query(User).filter_by(id=subscription.created_by).first()
                    if user.id in unique_user_ids:
                        sanitized_html = self.sanitize_and_transform(comment.comment_text)
                        ...
                        email_data = {
                            ...
                            "link": updated_link,
                            "type": "comment",
                            "message": f"New Message in App {self.app_dao.get_app_by_id(app_id=comment.app_id).name}",
                            "subject": f"New Message in App {self.app_dao.get_app_by_id(app_id=comment.app_id).name}",
                            "sanitized_html": sanitized_html,
                        }
                        ...
                        try:
                            ...
                            logging.exception(f"Failed to send email to {user.email_address}: {e}")
                elif subscription.subscription_setting == "off":
                    user = self.db_session.query(User).filter_by(id=subscription.created_by).first()
                    if user.id in unique_user_ids:
                        unique_user_ids.remove(user.id)
    ...
    async def send_reply_notifications(self, reply: Reply, updated_link: str):
        try:
            ...
            comment = self.get_comment_by_id(comment_id=reply.comment_id)
            if comment:
                thread_subscripiton = self.notification_subscription_dao.get_thread_subscription_by_comment_id(
                    comment_id=comment.id
                )
                for subscripiton in thread_subscripiton:
                    if subscripiton.subscription_setting:
                        user = self.db_session.query(User).filter_by(id=subscripiton.created_by).first()
                        screen_subscription = self.notification_subscription_dao.get_subscripition_by_user_id(
                            app_id=comment.app_id, screen_id=comment.app_screen_id, user_id=user.id
                        )
                        screen_subscription_setting = (
                            screen_subscription.subscription_setting if screen_subscription else None
                        )
                        if user.id in unique_user_ids and screen_subscription_setting == "off":
                            ...
                            email_data = {
                                ...
                                "link": updated_link,
                                "type": "comment",
                                "message": f"{reply.user_name} has mentioned you in a reply",
                                "subject": "You have been tagged in a reply",
                                "sanitized_html": sanitized_html,
                            }
                        ...
                        elif screen_subscription_setting == "off":
                            sanitized_html = self.sanitize_and_transform(reply.reply_text)
    ...
    def create_notification_objects(
        ...
        if type == "Task":
            ...
            for email in ordered_users:
                notifications_list.append(
                    Notifications(
                        ...
                        type=type,
                        link=updated_link,
                        approval_id=approval_id[ordered_users.index(email)],
                    )
                )
            ...
            notifications_list = []
            if comment_id:
                thread_subscription = self.notification_subscription_dao.get_thread_subscription_by_comment_id(
                    comment_id=comment_id
                )
                for subscription in thread_subscription:
                    if subscription.subscription_setting:
                        email = self.users_doa.get_user_by_id(user_id=subscription.created_by).email_address
                        screen_subscription = self.notification_subscription_dao.get_subscripition_by_user_id(
                            app_id=app_id, screen_id=screen_id, user_id=user_data.id
                        )
                        screen_subscription_setting = (
                            screen_subscription.subscription_setting if screen_subscription else None
                        )
                        if email not in ordered_users and screen_subscription_setting == "off":
                            notifications_list.append(
                                Notifications(
                                    alert_id=alert_id,
                                    app_id=app_id,
                                    widget_id=widget_id,
                                    title=title,
                                    message=f"New Message in App {self.app_dao.get_app_by_id(app_id=app_id).name}",
                                    is_read=is_read,
                                    user_email=email,
                                    widget_name=widget_name,
                                    shared_by=user_data.email_address,
            ...
            subscripitons = self.notification_subscription_dao.get_subscriptions_based_on_app_id(
                app_id=app_id, screen_id=screen_id
            )

            for subscripiton in subscripitons:
                if subscripiton.subscription_setting == "all comments":
                    email = self.users_doa.get_user_by_id(user_id=subscripiton.created_by).email_address
                    if email not in ordered_users:
                        notifications_list.append(
                            Notifications(
                                alert_id=alert_id,
                                app_id=app_id,
                        ...
                        ordered_users.remove(email)
                elif subscripiton.subscription_setting == "@mentions":
                    email = self.users_doa.get_user_by_id(user_id=subscripiton.created_by).email_address
                    if email in ordered_users:
                        notifications_list.append(
                            Notifications(
                        ...
                        ordered_users.remove(email)
                elif subscripiton.subscription_setting == "off":
                    email = self.users_doa.get_user_by_id(user_id=subscripiton.created_by).email_address
                    if email in ordered_users:
                        ordered_users.remove(email)
            for email in ordered_users:
                notifications_list.append(
                    Notifications(
                        ...
                        shared_by=user_data.email_address,
                        additional_info=additional_info,
                        type=type if type != "Alerts" else "Mentions",
                        link=updated_link,
                    )
                )
            return notifications_list
    ...
    def get_comment_by_id(self, comment_id):
        try:
            return self.db_session.query(Comment).filter_by(id=comment_id).first()
        except Exception as e:
            logging(e)
            raise e

    async def add_approval(self, approval_data: list[ApprovalAddSchema], comment_id):
        try:
            approval_ids = []
            for approval in approval_data:
                approval = Approval(comment_id, approval.user_id, approval.name, approval.status)
                self.db_session.add(approval)
                self.db_session.commit()
                self.db_session.refresh(approval)
                approval_ids.append(approval.id)
            return approval_ids
        except Exception as e:
            logging.exception(e)
            raise GeneralException(
                message={"error": "error while adding approval"},
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            )

    def get_approvals(self, comment_id):
        try:
            return self.db_session.query(Approval).filter_by(comment_id=comment_id).all()
        except Exception as e:
            logging.exception(e)
            raise GeneralException(
                message={"error": "error while getting approvals"},
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            )

    def edit_approval(self, user_id: int, request_data: ApprovalEditRequestSchema):
        try:
            approval = self.db_session.query(Approval).filter_by(id=request_data.approval_id).first()
            if approval:
                approval.status = request_data.status
                self.db_session.add(approval)
                self.db_session.commit()
                return approval
            else:
                raise GeneralException(
                    message={"error": "error while editing approval"},
    ...
    def get_approval_status_by_id(self, approval_id: int):
        try:
            approval_status = self.db_session.query(Approval).filter_by(id=approval_id).first()
            if approval_status:
            ...
            logging.exception(e)
            raise GeneralException(
                message={"error": "Approval status with this id"},
                status_code=status.HTTP_404_NOT_FOUND,
            )

    async def send_approval_notifications(self, approvals_list: list, comment: Comment, updated_link: str):
        try:
            for approval in approvals_list:
                user = self.db_session.query(User).filter_by(id=approval.user_id).first()
                sanitized_html = self.sanitize_and_transform(comment.comment_text)
                email_data = {
                    "email": user.email_address,
                    "link": updated_link,
                    "type": "comment",
                    "message": f"{comment.user_name} has assigned you a task",
