...
from fastapi import APIRouter, BackgroundTasks, Query, Request, Security, status
...
async def add_comment(
    ...
    return await comments_controller.add_comment(user_id, request_data, background_tasks)


@router.post("/app/screen/reply", status_code=status.HTTP_200_OK, response_model=ReplyAddResponseSchema)
@authenticate_user
async def add_reply(request: Request, request_data: ReplyAddRequestSchema, background_tasks: BackgroundTasks):
    """
    API to add comment
    """
    user_id = request.state.user.id
    return await comments_controller.add_reply(user_id, request_data, background_tasks)


@router.post("/filters", status_code=status.HTTP_200_OK, response_model=FilterAddResponseSchema)
@authenticate_user
async def filter_add(request: Request, request_data: FilterAddRequestSchema):
    """
    API to add comment
    """
...
@router.get(
    "/app/{app_id}/screen/{app_screen_id}", status_code=status.HTTP_200_OK, response_model=CommentsResponseSchema
)
@authenticate_user
async def get_comments(
    ...
    app_id: int,
    app_screen_id: int,
    widget_id: Optional[int] = Query(None, description="Widget ID, optional"),
...
async def get_filters_by_id(request: Request, id: int):
    """
...
@router.post("/approval/edit/", status_code=status.HTTP_200_OK, response_model=ApprovalEditResponseSchema)
@authenticate_user
async def edit_approval(
    ...
    """
    API to edit the approval status of the task
    """
    user_id = request.state.user.id
    return comments_controller.edit_approval(user_id=user_id, request_data=request_data)
