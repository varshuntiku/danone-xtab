...
from api.models import NacRoles, User, UserGroup, db
...
@bp.route("/codex-api/bulk/lists/users", methods=["GET"])
...
@bp.route("/codex-api/bulk/users", methods=["POST"])
    ...
    try:
        ...
        for index, row in ingested_df.iterrows():
            ...
            if user_email in deleted_users_list:
                ...
                if domain == "themathcompany.com":
                    restricted_access = True if restricted_user == "yes" else False
                    db.session.execute(
                        f"INSERT INTO public.user (created_at, first_name, last_name, email_address, access_key, restricted_user) VALUES (NOW(), '{user_first_name}', '{user_last_name}', '{user_email}', '{genAccessKey()}', {restricted_access} )"
                    )
                    ...
                    db.session.execute(
                        f"INSERT INTO public.user (created_at, first_name, last_name, email_address, access_key, restricted_user) VALUES (NOW(), '{user_first_name}', '{user_last_name}', '{user_email}', '{genAccessKey()}', 'True' )"
                    )
                    last_id_inserted = db.session.execute("SELECT LASTVAL()")
                    last_id_inserted = last_id_inserted.fetchone()[0]
                    db.session.commit()
