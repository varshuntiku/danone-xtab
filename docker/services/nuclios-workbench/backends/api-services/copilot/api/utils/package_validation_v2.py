...
def read_cache_packages(python_version="3_10"):
    try:
        compatible_packages_response = FileShareService(
            settings.AZURE_FILE_SHARE_CONNECTION_STRING, exec_env_repository, None
        ).get_text_file_data_from_specific_path(
...
def update_cache_packages(python_version="3_10", data={}):
    compatible_packages_response = FileShareService(
        settings.AZURE_FILE_SHARE_CONNECTION_STRING, exec_env_repository, None
    ).upload_file_as_data_to_specific_path(
        exec_env_repository,
