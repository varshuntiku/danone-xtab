...
TABLE_NAME = "execution_environment_deployment"
...
def test_model_structure_solutionbp_column_types(db_inspector):
    ...
    assert isinstance(columns["approval_status"]["type"], String)
    assert isinstance(columns["approved_by"]["type"], String)


def test_model_structure_category_nullable_constraints(db_inspector):
    columns = db_inspector.get_columns(TABLE_NAME)

    expected_nullable_columns = {"approved_by": True}
...
def test_model_structure_foreign_key(db_inspector):
    ...
    expected_foreign_key = {
        "constrained_columns": ["env_id"],
        "referred_table": "execution_environment",
        "referred_columns": ["id"],
    }
