...
def authenticate_user(func):
    ...
    async def wrapper(request, *args, **kwargs):
        ...
        # Uncomment this once we integrate sentry
...
def fetch_user_info(app_id, request, retry=0):
    global db_session
    try:
        return (
            db_session.query(AppUser)
            .filter(
                (fn.lower(AppUser.user_email) == fn.lower(request.state.logged_in_email))
        ...
        return fetch_user_info(app_id, request, retry + 1)
...
def app_user_info_required(func):
    ...
    async def wrapper(request, *args, **kwargs):
        ...
        if app_id:
            app_user = fetch_user_info(app_id, request)
            if app_user:
                request.state.app_user_info = app_user
                request.state.user_info["app_user"] = {
                    "responsibilities": (
                        json.loads(app_user.permissions).get("responsibilities", []) if app_user.permissions else None
                    )
                }
            else:
...
def environment_required(f):
    """Returns if the given user has super user and environment access.
