...
import { PublicAuthConnector } from "src/auth/AuthContext";
...
const getStore = async () => {
  if (!cachedStore) {
    cachedStore = (await import("store/store")).default;
  }
  return cachedStore;
};
...
const baseURL =
  (window?.envConfig?.global_style?.VITE_APP_ENABLE_FASTAPI ||
    import.meta.env["VITE_APP_ENABLE_FASTAPI"]) === "true"
    ? window?.envConfig?.global_style?.VITE_APP_NUCLIOS_BACKEND_API ||
      import.meta.env["VITE_APP_NUCLIOS_BACKEND_API"]
    : window?.envConfig?.global_style?.VITE_APP_BACKEND_API || import.meta.env["VITE_APP_BACKEND_API"];
...
export const generateAxiosClient = (
  ...
  baseurl = window?.envConfig?.global_style?.VITE_APP_NUCLIOS_BACKEND_API ||
    import.meta.env["VITE_APP_NUCLIOS_BACKEND_API"]
) => {
  ...
  retryCondition: (error) => {
    if (error.response) {
      if (error.response.status === 502) {
        ...
        (window?.envConfig?.global_style?.VITE_APP_ENV || import.meta.env["VITE_APP_ENV"]) === "prod"
      ) {
