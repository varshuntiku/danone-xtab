...
from api.models import AppScreenWidgetValue, db
...
bp = Blueprint("Planogram", __name__)


@bp.route(
    "/codex-product-api/app/<int:app_id>/<int:widget_value_id>/planogram",
    methods=["GET"],
)
@login_required
def get_planogram(app_id, widget_value_id):
    try:
        app_screen_widget_value = AppScreenWidgetValue.query.filter_by(id=widget_value_id).first()
...
@login_required
def save_planogram(app_id, widget_value_id):
    try:
        ...
        # app_screen_widget_value.widget_value = json.dumps(widget_value)
...
@bp.route("/codex-product-api/live-stream", methods=["GET"])
# @login_required
def get_live_feed():
    args = request.args.to_dict()
    try:
        url = args.get("live_feed")
...
def generate_user_message_rearrangement(planogram_json, arrangement_rules, dimensions, affinity_rules, affinity_values):
    msg = {
        "role": "user",
        "content": """
...
def generate_user_message_modification(action_command):
    msg = {"role": "user", "content": "Command: {}".format(action_command)}
    return msg


def calculate_dimensions_of_current_planogram(current_planogram):
    current_dimensions = []
...
def open_ai_call(open_ai_payload) -> ChatCompletion:
    # endpoint_url = (
    #     app.config["CHATGPT_OPENAI_BASE_URL"]
    #     + "/openai/deployments/"
    #     + app.config["CHATGPT_OPENAI_MODEL"]
    #     + "/chat/completions?api-version="
    #     + app.config["CHATGPT_OPENAI_API_VERSION"]
    # )
...
def get_llm_rearrangement_inference(request_data):
    planogram_json = {}
    analytics_file = {}
    sku_list = {}
    sku_data = pd.DataFrame()
    if len(request_data.get("analytics_file", [])) > 0:
        analytics_file = request_data.get("analytics_file")[0]
    ...
    arrangement_rules = request_data.get("rearrangement_rules")
    affinity_rules = request_data.get("sku_affinities")
    ...
    dimensions = calculate_dimensions_of_current_planogram(request_data.get("planogram"))
    openai_payload = {
        "messages": [
            rearranger_system_message,
            rearranger_user_example_message,
...
def get_llm_action_inference(command):
    openai_payload = {
        "messages": [
            modifier_system_message,
...
def get_image_for_sku(sku_id, original_sku_list):
...
def process_rearranged_planogram(gen_ai_output, original_payload):
    ...
    modified_payload = original_payload.get("planogram")
    modified_payload_planogram = modified_payload.get("rows")
    modified_payload_skus = original_payload.get("skus")
    try:
    ...
    modified_payload["status"] = {
        ...
        "updated_by": original_payload.get("user_name"),
    }
...
def process_modified_planogram(gen_ai_output, original_payload):
    original_planogram = original_payload.get("planogram")
...
@bp.route(
    "/codex-product-api/app/<int:app_id>/<int:widget_value_id>/planogram/genai/rearrange",
    methods=["POST"],
)
@login_required
def genai_rearrange(app_id: int, widget_value_id: int):
    try:
        ...
        return json_response({"planogram": processed_payload})
    except Exception as error_msg:
        ExceptionLogger(error_msg)
        # error_description = error_msg.args[0].get("error", "")
        return json_response(
            {
                "error": "Error: Could not connect to the GenAI Model",
...
@bp.route(
    "/codex-product-api/app/<int:app_id>/<int:widget_value_id>/planogram/genai/action_indetifier",
    methods=["POST"],
)
@login_required
def genai_action_identifier(app_id: int, widget_value_id: int):
    try:
        request_data = get_clean_postdata(request)
