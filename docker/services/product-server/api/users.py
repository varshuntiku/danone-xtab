...
from api.db_models.user_management.users import (
    NacRoles,
    User,
    UserGroup,
    UserPasswordCode,
...
from api.middlewares import login_required, nac_role_info_required, rbac_required
from api.models import App, AppUser, UserToken, app_user_role_identifier, db
from api.user_groups import list as user_group_list_fn
...
@bp.route("/codex-product-api/users/list", methods=["PUT"])
...
@bp.route("/codex-product-api/users/user-groups", methods=["GET"])
@login_required
# @rbac_required
def user_group_list():
...
@bp.route("/codex-product-api/users", methods=["POST"])
    ...
    try:
        ...
        if user is not None:
            ...
            user = User(
                ...
                if "restricted_access" in request_data and request_data["restricted_access"] != ""
                else False,
            )
            db.session.add(user)
            db.session.commit()
...
@bp.route("/codex-product-api/users/update", methods=["POST"])
@login_required
# @rbac_required
def update_for_app():
    try:
...
@bp.route("/codex-product-api/users/<int:user_id>", methods=["GET"])
@swag_from("./documentation/users/show.yml")
@login_required
# @rbac_required
def show(user_id):
    """Returns the user info for the given user id
    Args:
        user_id ([type]): [description]
    Returns:
        JSON : {id, first_name, last_name, email_address, user_groups, nac_user_roles, restricted_user}
    """
    try:
...
@bp.route("/codex-product-api/users/<int:user_id>", methods=["PUT", "POST"])
@swag_from("./documentation/users/update.yml", methods=["PUT"])
@login_required
@rbac_required
def update(user_id):
    """Updates the user info for the given user id.
    Args:
        user_id ([type]): [description]
    Returns:
        json : {'status': True}
    ...
    try:
        ...
        if user is None:
            ...
            user.restricted_access = (
                request_data["restricted_access"]
                if "restricted_access" in request_data and request_data["restricted_access"] != ""
                else False
            )
            user.updated_by = g.user.id
...
@bp.route("/codex-product-api/users/<int:user_id>", methods=["DELETE"])
@swag_from("./documentation/users/delete.yml")
@login_required
@rbac_required
def delete(user_id):
    """Deletes the user info for the given user id.
    Args:
        user_id ([type]): [description]
    Returns:
        JSON : {'deleted_rows': 1}
...
@bp.route("/codex-product-api/delete_users", methods=["POST"])
@swag_from("./documentation/users/delete.yml")
@login_required
@rbac_required
def delete_multiple_users():
    """Deletes the users info for the given list of user ids.
    Args:
    ...
    try:
        user_email = db.session.query(func.lower(User.email_address)).filter(User.id.in_(user_ids))
        user = db.session.query(User).filter(User.id.in_(user_ids))
        user.update({"deleted_at": current_time, "deleted_by": g.user.id})
        db.session.commit()
        send_email_smtp(
            email_type="",
            To=[email[0] for email in user_email],
            Subject="Nuclios access revoked",
            body={
                "plain": f"Dear user, Your access to Nuclios was revoked by {g.user.first_name} as you have not accesed Nuclios for the last six months.If you think this was a mistake Please reach out to {g.user.email_address}/nRegards,/nTeam Nuclios"
            },
        )
        return json_response({"deleted_rows": len(user_ids)})
    except Exception as error_msg:
        db.session.rollback()
        ExceptionLogger(error_msg)
        return json_response({"Error": "Error deleting users"}, 500)
...
@bp.route("/codex-product-api/user-apps/<email_address>", methods=["GET"])
@swag_from("./documentation/dashboard/get_all_user_apps.yml")
@login_required
# @rbac_required
def get_user_apps(email_address):
    try:
        response = [
            {"id": data.app_id, "name": data.name, "app_user_id": data.id}
            for data in AppUser.query.filter_by(user_email=email_address)
...
# segregate the apps in hjere and work on it as post method needs to be used
@bp.route("/codex-product-api/user-apps/<email_address>", methods=["POST"])
@swag_from("./documentation/dashboard/update_user_apps.yml")
@login_required
# @rbac_required
def update_user_apps(email_address):
    try:
        request_data = get_clean_postdata(request)
        first_name = request_data.get("first_name") if request_data.get("first_name") else None
        last_name = request_data.get("last_name") if request_data.get("last_name") else None
        # user_id = request_data.get("user_id") if request_data.get("user_id") else None
        default_apps = (
            set(request_data.get("default_apps"))
            if request_data.get("default_apps")
            and all(isinstance(i, int) for i in request_data.get("default_apps"))
            and len(request_data.get("default_apps")) != 0
            else set()
        )
        user_apps = (
            set(request_data.get("user_apps"))
            if request_data.get("user_apps")
            and all(isinstance(i, int) for i in request_data.get("user_apps"))
            and len(request_data.get("user_apps")) != 0
            else set()
        )
        # print(default_apps,user_apps)
        removable_apps = default_apps - user_apps
        create_apps = user_apps - default_apps
        # print(removable_apps,create_apps)
        ...
        if len(removable_apps) != 0:
            ...
            for application_id in removable_apps:
                try:
                    app_user = (
                        AppUser.query.filter_by(user_email=email_address.lower())
                        .filter_by(app_id=application_id)
                        .first()
                    )
                    if app_user is None:
                        return json_response({"error": "App User not found"}, 404)
                        ...
                        db.session.query(app_user_role_identifier).filter_by(app_user_id=app_user.id).delete()
        ...
        if len(create_apps) != 0:
            ...
            for application in create_apps:
                try:
                    app_user = (
                        AppUser.query.filter_by(user_email=email_address.lower()).filter_by(app_id=application).first()
                    )
...
@bp.route("/codex-product-api/user/token/", methods=["POST"])
# TODO: uncomment swagger decorator once the documention file is added
# @swag_from("../../documentation/alerts/alerts-JWT_token.yml")
@login_required
...
def generate_jwt_token():
    ...
    useremail = request_data.get("user_email")
    ...
    try:
        ...
        user_token = UserToken(None, username, useremail, token, json.dumps(access))
        db.session.add(user_token)
        db.session.flush()
        db.session.commit()
        return json_response({"message": "created JWT_token successfully", "token": token}, 200)

    except Exception as ex:
        ExceptionLogger(ex)
        db.session.rollback()
        return json_response({"error": "Error while generating JWT token"}, 500)

    finally:
        db.session.close()


@bp.route("/codex-product-api/user/token/", methods=["GET"])
# TODO: uncomment swagger decorator once the documention file is added
# @swag_from("../../documentation/alerts/get-alerts-JWT_tokens.yml")
@login_required
@nac_role_info_required
def get_jwt_tokens():
    try:
        jwt_tokens_by_user = (
            UserToken.query.filter(
                ...
                UserToken.user_email == g.logged_in_email,
            )
        ...
        response = [
            {
                ...
                "token": detail.execution_token,
                "access": detail.access,
            }
            for detail in jwt_tokens_by_user
        ]
        return json_response({"message": "token fetched", "tokens": response}, 200)

    except Exception as ex:
        ExceptionLogger(ex)
        return json_response({"error": "Error while fetching tokens"}, 500)


@bp.route("/codex-product-api/user/token/<int:id>", methods=["DELETE"])
# TODO: uncomment swagger decorator once the documention file is added
# @swag_from("../../documentation/alerts/delete_jwt_token.yml")
@login_required
@nac_role_info_required
def delete_jwt_token(id):
    try:
        usertoken = UserToken.query.filter_by(id=id).first()
        db.session.delete(usertoken)
        db.session.flush()
        db.session.commit()
        return json_response({"message": "Deleted the token successfully"}, 200)
    except Exception as ex:
        ExceptionLogger(ex)
        return json_response({"error": "Error while deleting token"}, 500)


@bp.route("/codex-product-api/users-email", methods=["GET"])
# @swag_from("./documentation/users/list.yml")
# @login_required
# @rbac_required
def user_email():
...
@bp.route("/codex-product-api/user/generate-code", methods=["POST"])
...
@bp.route("/codex-product-api/user/validate-otp", methods=["POST"])
...
@bp.route("/codex-product-api/user/change-password", methods=["POST"])
...
@bp.route("/codex-product-api/user/update-password", methods=["POST"])
...
def generate_otp_mail(code, user_data):
    try:
        ...
        html_path = Path(__file__).parent / "./email-templates/forgot-password.html"
        ...
        html_template.close()
        plain_text_path = Path(__file__).parent / "./email-templates/forgot-password.txt"
