...
def upgrade():
    ...
    op.create_table('copilot_orchestrator',
    ...
    op.alter_column('minerva_conversation', 'feedback',
    ...
    op.create_foreign_key('copilot_app_orchestrator_id_fkey', 'copilot_app', 'copilot_orchestrator', ['orchestrator_id'], ['id'])
    op.create_table('copilot_app_datasource_published_tool_mapping',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True),
    ...
    sa.Column('updated_by', sa.Integer(), nullable=True),
    sa.Column('deleted_by', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['app_published_tool_id'], ['copilot_app_published_tool_mapping.id'], ),
    sa.ForeignKeyConstraint(['created_by'], ['user.id'], ),
    ...
    op.alter_column('copilot_app_published_tool_mapping', 'desc',
               existing_type=sa.VARCHAR(length=100),
               type_=sa.Text(),
               existing_nullable=True)
    op.alter_column('copilot_tool', 'desc',
               existing_type=sa.VARCHAR(length=100),
               type_=sa.Text(),
               existing_nullable=True)
    op.alter_column('copilot_tool_registry', 'desc',
               existing_type=sa.VARCHAR(length=100),
               type_=sa.Text(),
               existing_nullable=True)
    ...
    op.create_table('copilot_tool_version_orchestrator_mapping',
    sa.Column('tool_version_id', sa.Integer(), nullable=True),
    sa.Column('orchestrator_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['orchestrator_id'], ['copilot_orchestrator.id'], ),
    sa.ForeignKeyConstraint(['tool_version_id'], ['copilot_tool_version.id'], )
    )
    op.execute('''INSERT INTO copilot_tool_version_orchestrator_mapping (tool_version_id, orchestrator_id)
SELECT tv.id, o.id
    ...
    try:
        ...
        for tool in existing_tools:
            if tool.config == {} or tool.config is None:
                ...
                new_tool_datasource_mapping_config = {}     # create a config obj for copilot_app_datasource_published_tool_mapping
                updated_app_published_tool_config = {       # create an updated config json for app_published_tool_mapping table
                        **tool.config
                }
                if tool.config.get('datasource', False):
                    tool_ds_id = tool.config.get('datasource', False)
                    # check if datasource exist
                    datasource_res = connection.execute(f'''
                            SELECT *
