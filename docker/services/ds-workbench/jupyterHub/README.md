...
## JupyterHub Secrets
When deploying a fresh JupyterHub on kubernetes Cluster, apply the secrets.yaml file to the cluster with the following contents

apiVersion: v1
kind: Secret
metadata:
  name: dsworkbench-secret
  namespace: "{{ .Release.Namespace }}" # should come from environment, no hard coding
type: Opaque
data:
  sharename: <base64 encoded>
