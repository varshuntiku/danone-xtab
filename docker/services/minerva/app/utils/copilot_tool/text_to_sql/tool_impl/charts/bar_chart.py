    ...
    def getChart(self):
        ...
        if self.chartType == "bar":
            ...
            if self.chartType in ["stacked bar", "horizontal stacked"]:
                ...
                barmode = "group"
            numericalCol = next(col for col in self.metaData if col["data_type"] == "continuous")
            catCol1 = next(col for col in self.metaData if col["data_type"] == "categorical")
            catCol2 = next(
                col for col in self.metaData if (col["data_type"] == "categorical" and col["name"] != catCol1["name"])
            )
            xaxisTitle = (
                numericalCol["name"]
                if (
