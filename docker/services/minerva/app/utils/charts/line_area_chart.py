...
class LineAreaChart(BaseChart):
    ...
    def getChart(self):
        ...
        numericalCol = next(
            col for col in self.metaData if (col["dataType"] == "continuous" and col["dataValue"] != "time")
        )
        ...
        if self.chartType == "line":
            ...
            if self.chartType in ["multiline-format", "area-format"]:
                ...
                catCol = next(
                    col for col in self.metaData if (col["dataType"] == "categorical" and col["dataValue"] != "time")
                )
