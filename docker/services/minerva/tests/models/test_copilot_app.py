...
TABLE_NAME = "copilot_app"


def test_copilot_app_table_exists(db_inspector):
...
def test_copilot_app_table_column_types(db_inspector):
    ...
    assert isinstance(columns["orchestrator_config"]["type"], JSON)
    assert isinstance(columns["orchestrator_id"]["type"], Integer)
...
def test_copilot_app_table_nullable_constraints(db_inspector):
...
def test_copilot_app_table_col_lengths(db_inspector):
    columns = {columns["name"]: columns for columns in db_inspector.get_columns(TABLE_NAME)}
    assert columns["name"]["type"].length == 100


def test_copilot_app_table_foreign_key(db_inspector):
