...
TABLE_NAME = "copilot_conversation_datasource"
...
def test_model_column_types(db_inspector):
    ...
    assert isinstance(columns["meta"]["type"], JSON)
    assert isinstance(columns["type"]["type"], Text)
...
def test_model_structure_foreign_key(db_inspector):
    ...
    expected_foreign_key = {
        ...
        "referred_table": "copilot_conversation",
        "referred_columns": ["id"],
    }
