...
import AttachmentPreview from "views/Widgets/AttachmentPreview.jsx";

import CodexDataProvider, { CODEX_API_GET, CODEX_API_DELETE } from "views/CRUD/CodexDataProvider.jsx";
...
class Attachments extends React.Component {
  ...
  refreshData = () => {
    ...
    if (widget_id) {
      ...
      CodexDataProvider(
        CODEX_API_GET,
        {
          resource: "widgets/get-artifacts",
          action: widget_id,
  ...
  getFilePreview = (file_data) => {
    const { classes, widget_id, readonly } = this.props;

    return <AttachmentPreview parent_obj={this} classes={classes} widget_id={widget_id} file_data={file_data} readonly={readonly} />
  }

  onClickDeleteFile = (attachment_id) => {
    var resource_url = "widgets/delete-artifact";
      ...
      <div style={{ padding: '5px', position: 'relative' }}>
        ...
        <div>
          {_.map(this.state.files, function(file_data, file_index) {
            return (
              <div key={"widget_artifact_preview_" + file_index} style={{ float: 'left', marginRight: '5px', position: 'relative' }}>
