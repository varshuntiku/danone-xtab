...
const {DARK_THEME} =require('../enum');
function Context(options) {
    this.indent = -1;

    this.fontFace = options.fontFace || 'Roboto Condensed Light';
...
Context.prototype.parseSize = function (value, stack) {
    let parts = value.trim().match(/([\d\.]+)(%|em|pt|rem|px)/);
    let scale;
...
Context.prototype.toPptxTextOptions = function () {
    ...
    (this.b || this.strong) &&(options.bold = !!this.b || !!this.strong);
    this.break && (options.breakLine = !!this.break);
    options.color = this.color?this.color:DARK_THEME.DARK_THEME_FONT_COLOR;
    this.fill && (options.highlight = this.fill);
    this.fontFace && (options.fontFace = this.fontFace);
    (this.fontSize || this.defaultFontSize)&&(options.fontSize = this.fontSize || this.defaultFontSize);
    (this.i || this.em) &&(options.italic = !!this.i || !!this.em);
    this.shadow && (options.shadow = this.shadow);
    this.s && (options.strike = 'sngStrike');
    this.sub &&(options.subscript = !!this.sub);
    this.sup && (options.superscript = !!this.sup);
    (this.u || this.ins) &&(options.underline = !!this.u || !!this.ins);
