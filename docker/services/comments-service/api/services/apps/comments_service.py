...
class CommentsService(BaseService):
    ...
    async def add_comment(
        ...
        new_comment = await self.comments_dao.add_comment(request, request_data, background_tasks)
        return new_comment.id

    async def add_reply(
        self, request, user_email: str, request_data: ReplyAddRequestSchema, background_tasks: BackgroundTasks
    ):
        new_reply = await self.comments_dao.add_reply(request, user_email, request_data, background_tasks)
        return new_reply.id
    ...
    def delete_state_change(self, user_id: str, comment_id: int, type: str):
